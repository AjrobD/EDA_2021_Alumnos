...
public class LinkedBinarySearchTreePrueba<E> implements BinarySearchTree<E> {
	 ...
	 */
	public LinkedBinarySearchTreePrueba() {
		this(null);
	}
	 ...
	 */
	public LinkedBinarySearchTreePrueba(Comparator<E> c) {
		if (c == null) {
...
//    }
	...
	public Position<E> first() throws RuntimeException {
		Position<E> aux = binTree.root();
		while(binTree.left(aux).getElement()!=null){
			aux = binTree.left(aux);
		}
		return aux;
	}
	...
	public Position<E> last() throws RuntimeException {
		Position<E> aux = binTree.root();
		while(binTree.right(aux).getElement()!=null){
			aux = binTree.right(aux);
		}
		return aux;
	}

	public Iterable<Position<E>> successors(Position<E> pos){
		ArrayList<Position<E>> sucesores = new ArrayList<>();
		recursiveSuccessors(pos,binTree,sucesores,false);
		return sucesores;
	}

	private void recursiveSuccessors(Position<E> pos, LinkedBinaryTree<E> binTree, ArrayList<Position<E>> sucesores, Boolean fromRight) {
		if(!fromRight){
			sucesores.add(pos);
			if(binTree.right(pos).getElement()!=null)
				addToListUnder(binTree.right(pos),binTree,sucesores,false);
		}
		if(!binTree.isRoot(pos)){
			fromRight = pos.equals(binTree.right(binTree.parent(pos)));
			recursiveSuccessors(binTree.parent(pos),binTree,sucesores,fromRight);
		}
	}

	private void addToListUnder(Position<E> pos, LinkedBinaryTree<E> binTree, ArrayList<Position<E>> sucesores, Boolean lesser) {
		if(lesser){
			if(binTree.right(pos).getElement()!=null){
				addToListUnder(binTree.right(pos),binTree,sucesores,lesser);
			}
			sucesores.add(pos);
			if(binTree.left(pos).getElement()!=null){
				addToListUnder(binTree.left(pos),binTree,sucesores,lesser);
			}
		}
		else{
			if(binTree.left(pos).getElement()!=null){
    ...
    public Iterable<Position<E>> findRange(E minValue, E maxValue) throws RuntimeException{
		List<Position<E>> rango = new ArrayList<>();
		findRangeRecursive(minValue,maxValue,rango,binTree.root());
		return rango;
    }

	private void findRangeRecursive(E minValue, E maxValue, List<Position<E>> rango, Position<E> pos) {
    	if(comparator.compare(pos.getElement(),minValue)>=0){
    		if(binTree.left(pos).getElement()!=null){
    			findRangeRecursive(minValue,maxValue,rango,binTree.left(pos));
			}
		...
		if(comparator.compare(pos.getElement(),maxValue)<=0){
			if(binTree.right(pos).getElement()!=null){
				findRangeRecursive(minValue,maxValue,rango,binTree.right(pos));
			}
		}
	...
	/**
class ReestructurableBinaryTree<T> extends LinkedBinaryTreePrueba<T>{
